...
export const editReplaceExactTool: ToolDefinition<EditReplaceExactInput, EditPrepareResult> = {
  ...
  description: "Replace exact text in a file. The 'old' text must match exactly (including whitespace). Use read_file first to get the exact content.",
  ...
  inputSchema: {
    ...
    properties: {
      ...
      expectedOccurrences: {
        type: "number",
        description: "Expected number of occurrences. If provided, must match exactly or the operation fails.",
      },
    },
    required: ["path", "old", "new"],
  },

  async execute(args: EditReplaceExactInput, ctx: ToolContext): Promise<ToolResult<EditPrepareResult>> {
