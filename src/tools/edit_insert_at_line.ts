...
export const editInsertAtLineTool: ToolDefinition<EditInsertAtLineInput, EditPrepareResult> = {
  ...
  description: "Insert content at a specific line number. The content will be inserted BEFORE the specified line. Line numbers are 1-based.",
  ...
  inputSchema: {
    ...
    properties: {
      ...
      line: {
        type: "number",
        description: "Line number to insert before (1-based). Use 1 to insert at the beginning, or file length + 1 to append.",
      },
      content: {
        type: "string",
  ...
  async execute(args: EditInsertAtLineInput, ctx: ToolContext): Promise<ToolResult<EditPrepareResult>> {
    ...
    if (args.line > lines.length + 1) {
      return { ok: false, error: `Line ${args.line} exceeds file length (${lines.length} lines). Use ${lines.length + 1} to append.` };
    }
