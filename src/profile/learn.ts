...
function looksLikePlanning(text: string): boolean {
    ...
    return planningPrefixes.some((prefix) => lower.startsWith(prefix));
}
...
async function queryWithTools(
    ...
    const systemPrompt =
        "You are helping North learn about a codebase. Use tools to explore the code. After exploring, provide ONLY the final concise summary with bullet points. Be direct and factual. Do NOT narrate what you will do - just do it silently and return the answer.";
    ...
    if (needsSummary && didUseTools) {
        conversationMessages.push({
            role: "user",
            content:
                "Now provide ONLY the final answer based on what you found. Use bullet points. Do not explain what you did - just give the summary.",
        });
    ...
    if (finalText.trim() && !looksLikePlanning(finalText)) {
        return finalText.trim();
    }

    if (looksLikePlanning(finalText)) {
        return "";
    }
